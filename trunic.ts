...
let total_seen: number = 0

let current_cons: string = undefined
let has_guessed_cons: boolean = false
let cons_correct: number = 0

let current_vowel: string = undefined
let has_guessed_vowel: boolean = false
let vowel_correct: number = 0
...
function set_rune(toggles: string[]): void {
...
function guess_consonant(g: string): void {
    if (current_cons === g) {
...
function guess_vowel(g: string): void {
    if (current_vowel === g) {
        document.getElementById(g).className = "correct"
